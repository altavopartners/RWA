...
async function main() {
    ...
    const order2 = await prisma.order.create({
        data: {
            ...
            userId: restaurantBuyer.id,
            status: client_1.OrderStatus.BANK_REVIEW,
            subtotal: 30000,
            shipping: 800,
            total: 30800,
            buyerBankId: franceBank.id,
    ...
    for (const doc of order2Docs) {
        ...
        await prisma.document.create({
            data: {
                ...
                category: doc.cat,
                documentType: client_1.DocumentType.OTHER,
                status: client_1.DocumentStatus.VALIDATED,
    ...
    for (const doc of order5Docs) {
        ...
        await prisma.document.create({
            data: {
                ...
                documentType: client_1.DocumentType.OTHER,
                status: doc.status,
                validatedBy: doc.status !== client_1.DocumentStatus.PENDING ? "John Smith" : undefined,
                validatedAt: doc.status !== client_1.DocumentStatus.PENDING
