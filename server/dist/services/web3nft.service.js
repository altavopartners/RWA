...
exports.createProductNFT = createProductNFT;
const sdk_1 = require("@hashgraph/sdk");
const hedera_1 = require("../config/hedera");
async function createProductNFT(input) {
    const client = (0, hedera_1.getHederaClient)();
    ...
    const supplyKey = process.env.HEDERA_SUPPLY_KEY
        ? sdk_1.PrivateKey.fromString(process.env.HEDERA_SUPPLY_KEY)
        : sdk_1.PrivateKey.generateED25519();
    // If you generated it here, persist it securely (KMS/secret store) for future mints.
    const symbol = input.name.replace(/[^a-zA-Z0-9]/g, "").substring(0, 5).toUpperCase() || "PROD";
    let tokenId = "";
    try {
    ...
    function fitUtf8(text, maxBytes) {
        ...
        while (lo < hi) {
            ...
            if (Buffer.from(slice, "utf8").byteLength <= maxNoEllipsis)
                lo = mid;
            else
                hi = mid - 1;
        }
    ...
    function makeTextMetadata(i, total, input) {
    ...
    for (let i = 0; i < mintCount; i += chunkSize) {
        const batch = Array.from({ length: Math.min(chunkSize, mintCount - i) }, (_, k) => makeTextMetadata(i + k, mintCount, input));
        try {
            const mintTx = new sdk_1.TokenMintTransaction()
                .setTokenId(tokenId)
                .setMetadata(batch)
                .setMaxTransactionFee(new sdk_1.Hbar(20)) // ensure enough fee for batch mints
                .freezeWith(client);
...
//# sourceMappingURL=web3nft.service.js.map