...
const REQUIRED_FIELDS: (keyof FormState)[] = ["name", "quantity", "unit", "pricePerUnit", "countryOfOrigin", "category", "description"];
...
const ProducerAddProductPage = () => {
  ...
  useEffect(() => {
    ...
    const checkConnection = async () => {
      // Add a small delay to allow auth state to rehydrate
  ...
  // Show loading state while checking (only on initial mount)
  if (isChecking) {
    return (
...
const ProducerAddProductPageContent = () => {
  ...
  const filteredCountries = useMemo(() => {
    if (!countrySearch) return africanCountries;
    return africanCountries.filter(country => 
      country.toLowerCase().startsWith(countrySearch.toLowerCase())
    );
  }, [countrySearch]);

  const availableSubcats = useMemo(() => (form.category ? subcategories[form.category] : []), [form.category]);

  const setField = (key: keyof FormState, value: any) => setForm((s) => ({ ...s, [key]: value }));
  ...
  const validate = (f: FormState): Errors => {
    ...
    for (const k of REQUIRED_FIELDS) {
      ...
      if (v === undefined || v === null || String(v).trim() === "") e[k] = "This field is required.";
    }
    if (!f.images || f.images.length < 1) e.images = "Please upload at least one image.";
    if (f.quantity && (!/^\d+$/.test(f.quantity) || Number.parseInt(f.quantity) <= 0)) e.quantity = "Quantity must be a positive integer.";
    if (f.pricePerUnit && (isNaN(Number(f.pricePerUnit)) || Number(f.pricePerUnit) <= 0)) e.pricePerUnit = "Price per unit must be a positive number.";
    if (f.description && f.description.trim().length < 20) e.description = "Please add more detail (min 20 characters).";
    return e;
  };
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      setSubmitting(true);
      const res = await fetch("http://localhost:4000/api/products", { method: "POST", body: fd });
      ...
      <div className="container mx-auto max-w-7xl px-6 -mt-8 relative z-20">
        ...
        <Card className="p-8 md:p-10 bg-white dark:bg-gray-900 border-gray-200 dark:border-gray-800 shadow-2xl">
          <form onSubmit={handleSubmit} className="space-y-8">
            ...
            <div className="space-y-6">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                <div className="lg:col-span-2">
                  ...
                  <Input id="name" placeholder="e.g., Premium Ethiopian Coffee Beans" value={form.name} onChange={handleChange("name")} className={`mt-2 ${errors.name ? 'border-red-500' : ''}`} />
                ...
                <div>
                  ...
                  <Input id="quantity" type="number" placeholder="1000" value={form.quantity} onChange={handleChange("quantity")} className={`mt-2 ${errors.quantity ? 'border-red-500' : ''}`} />
                ...
                <div>
                  ...
                  <div className="relative mt-2">
                    ...
                    <Input id="pricePerUnit" type="number" step="0.01" placeholder="2.50" value={form.pricePerUnit} onChange={handleChange("pricePerUnit")} className={`pl-10 ${errors.pricePerUnit ? 'border-red-500' : ''}`} />
                  </div>
            ...
            <div className="space-y-6">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                <div>
                  ...
                  <Select value={form.category} onValueChange={(v: string) => setForm((s) => ({ ...s, category: v as FormState["category"], subcategory: undefined }))}>
                    <SelectTrigger className={`mt-2 ${errors.category ? 'border-red-500' : ''}`}><SelectValue placeholder="Select a category" /></SelectTrigger>
                ...
                <div>
                  ...
                  <Select value={form.subcategory} onValueChange={(v) => setField("subcategory", v)} disabled={!form.category}>
                    <SelectTrigger className="mt-2"><SelectValue placeholder={form.category ? "Select subcategory" : "Choose category first"} /></SelectTrigger>
                    <SelectContent>{availableSubcats.map((sc) => (<SelectItem key={sc.id} value={sc.id}>{sc.label}</SelectItem>))}</SelectContent>
                  </Select>
                </div>
                ...
                <div className="md:col-span-2 lg:col-span-3">
                  ...
                  <Textarea id="description" rows={4} placeholder="Describe your product, quality standards, certifications..." value={form.description} onChange={handleChange("description")} className={`mt-2 ${errors.description ? 'border-red-500' : ''}`} />
            ...
            <div className="space-y-6">
                  ...
                  <div className="mt-4 grid grid-cols-2 md:grid-cols-4 gap-4">
                    {imageFiles.map((file, idx) => (
                      <div key={idx} className="relative group rounded-lg overflow-hidden border">
                        ...
                        <button type="button" onClick={() => setField("images", imageFiles.filter((_, i) => i !== idx))} className="absolute top-2 right-2 bg-white hover:bg-red-500 text-gray-900 hover:text-white rounded-full p-1.5 opacity-0 group-hover:opacity-100 transition-all">
                          <X className="h-4 w-4" />
                        </button>
                      </div>
                    ))}
              ...
              <div>
                ...
                <div className="mt-2 border-2 border-dashed rounded-xl p-8 text-center transition-all hover:border-[#88CEDC] dark:hover:border-[#88CEDC] border-gray-300 dark:border-gray-700 bg-gray-50 dark:bg-gray-800/50">
                  ...
                  <Button type="button" variant="outline" onClick={() => docsRef.current?.click()}><Upload className="w-4 h-4 mr-2" />Upload Documents</Button>
                </div>
            ...
            <Button type="submit" size="lg" className="w-full bg-gradient-to-r from-[#88CEDC] to-[#5BA8B8] hover:from-[#7BC0CF] hover:to-[#4A97A7] text-white font-bold py-6" disabled={submitting || (form.images?.length ?? 0) < 1}>
              {submitting ? (<><Sparkles className="w-5 h-5 mr-2 animate-spin" />Creating Your Product...</>) : (<><Sparkles className="w-5 h-5 mr-2" />List Product on Marketplace</>)}
            </Button>
